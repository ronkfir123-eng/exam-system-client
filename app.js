...
addQuestionBtn.addEventListener("click", () => {
  ...
  if (!title) return examUI.showBuilderMessage("Please enter exam title.", "danger");
  if (!questionText) return examUI.showBuilderMessage("Please enter question text.", "danger");
  if (answers.some(answer => answer === "")) return examUI.showBuilderMessage("Please fill all 4 answers.", "danger");
  if (correctAnswerNumber < 1 || correctAnswerNumber > 4) return examUI.showBuilderMessage("Correct answer must be a number from 1 to 4.", "danger");
  ...
  examUI.showBuilderMessage(`Question added. Current exam has ${currentExam.getQuestionCount()} question(s).`, "success");
  clearQuestionInputs();
});

saveExamBtn.addEventListener("click", () => {
  if (!currentExam) return examUI.showBuilderMessage("Create an exam and add at least one question first.", "danger");
  if (currentExam.getQuestionCount() === 0) return examUI.showBuilderMessage("Cannot save exam without questions.", "danger");
